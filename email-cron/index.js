...
//             const savedData = await CustomerEmail.create({
//               CustomerId: customer.Id,
...
//             console.log("saved info", savedData);
...
const fetchEmailsFromFolder = async (imap, folder, lastFetchedDate) => {
  return new Promise((resolve, reject) => {
    imap.openBox(folder, false, (err) => {
      ...
      console.log("SearchCritera",searchCriteria)
      ...
      imap.search(searchCriteria, (err, results) => {
            ...
            new Promise((resolveFetch, rejectFetch) => {
              msg.on("body", (stream) => {
                simpleParser(stream, (err, parsed) => {
                  ...
                  logger.info("Fetched email:", emailData);
        ...
        f.once("end", async () => {
          try {
            ...
            console.error(`Error completing fetch for ${folder}:`, error);
            logger.error(`Error completing fetch for ${folder}:`, error);
            reject(error);
          }
        });
...
const processEmails = async (store, emails) => {
  ...
  return new Promise((resolve, reject) => {
    imap.once("ready", async () => {
      try {
        ...
        await processEmails(store, inboxEmails);
